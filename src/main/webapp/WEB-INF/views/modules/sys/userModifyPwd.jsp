...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 添加自定义验证方法
            $.validator.addMethod("notSimplePassword",function(value,element,params){
                return !isSimplePwd(value);
            },"密码必须大于等与8位并包含字母和数字！");

			$("#inputForm").validate({
				rules: {
				    newPassword: {notSimplePassword: "密码必须大于等与8位并包含字母和数字！"}
				},
				messages: {
				    newPassword:{notSimplePassword : "密码必须大于等与8位并包含字母和数字！"},
...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/modifyPwd" method="post" class="form-horizontal">
        ...
        <div class="control-group">
            <label class="lbl">根据风险部需求，系统将强制您修改您之前使用的简单密码。新密码需要至少8位，且包含数字和字母。
                修改后重新登录即可。
            </label>
        </div>
		<div class="control-group">
